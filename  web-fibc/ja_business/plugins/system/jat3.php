...
class plgSystemJAT3 extends JPlugin {
	...
	function onAfterRoute() {
		...
		if (! $app->isAdmin () && T3Common::detect ()) {
			...
			if (T3Common::isRTL() && is_dir(T3Path::path (T3_TEMPLATE_CORE.'/themes/default-rtl'))) $path->addPath ( 'core.default-rtl', T3Path::path (T3_TEMPLATE_CORE.'/themes/default-rtl'), T3Path::url (T3_TEMPLATE_CORE.'/themes/default-rtl') );
			$path->addPath ( 'template.default', T3Path::path (T3_TEMPLATE), T3Path::url (T3_TEMPLATE) );
			if (T3Common::isRTL() && is_dir(T3Path::path (T3_BASETHEME.'-rtl'))) $path->addPath ( 'engine.default-rtl', T3Path::path (T3_BASETHEME.'-rtl'), T3Path::url (T3_BASETHEME.'-rtl') );
