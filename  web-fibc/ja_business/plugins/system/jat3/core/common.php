...
class T3Common {
	...
	function layout_exists ($layout) {
		...
		//check in local path
		if (T3Common::isRTL() && is_file (T3Path::path(T3_TEMPLATE_LOCAL).DS.$pathrtl)) return T3Path::path(T3_TEMPLATE_LOCAL).DS.$pathrtl;
		if (is_file (T3Path::path(T3_TEMPLATE_LOCAL).DS.$path)) return T3Path::path(T3_TEMPLATE_LOCAL).DS.$path;
		//check in core path
		if (T3Common::isRTL() && is_file (T3Path::path(T3_TEMPLATE_CORE).DS.$pathrtl)) return T3Path::path(T3_TEMPLATE_CORE).DS.$pathrtl;
		if (is_file (T3Path::path(T3_TEMPLATE_CORE).DS.$path)) return T3Path::path(T3_TEMPLATE_CORE).DS.$path;
		//check in engine path
		if (T3Common::isRTL() && is_file (T3Path::path(T3_BASETHEME.'-rtl').DS.$path)) return T3Path::path(T3_BASETHEME.'-rtl').DS.$pathrtl;
		...
		if (!isset($_themes)) {
			...
			for ($i=0;$i<count($themes);$i++) {
				...
				if (T3Common::isRTL()) {
					...
					if ($themertl[1] && is_dir (T3Path::path(T3_TEMPLATE).DS.$themertl[0].DS.'themes'.DS.$themertl[1])) {
						$_themes[] = $themertl; 
					}
				}
